...
2. A **facet** is a contract with one or more functions to be called from a diamond. The term facet comes from the diamond industry. It is a side, or flat surface of a diamond.
3. A **loupe facet** is a facet contract that provides introspection functions. In the diamond industry, a loupe is a magnifying glass that is used to look at diamonds.
4. An **immutable function** is a function that cannot be replaced or removed (because it is defined directly in the diamond, or because the diamond's logic does not allow it to be modified).
...
Facets implementing a required interface should be available after the diamonds constructor completes.
...
A state variable or storage layout organizational pattern is needed because Solidity's builtin storage layout system doesn't support proxy contracts or diamonds. The particular layout of storage is not defined in this EIP, but may be defined by later proposals. Facets and diamonds following one layout are unlikely to be compatible with facets and diamonds following another layout.
...
### Modifying Facets & Functions

A diamond may implement one or both of the interfaces below, and expose functions that update the mapping of function selector to facet address.

Diamonds that allow modifications to their function selector mapping must implement the `IDiamondMutable` interface, and emit the `DiamondCut` event on any change.

Diamonds that allow modifications to their function selector mapping should implement the `IDiamondCut` interface.

#### `IDiamondMutable` Interface

All diamonds that allow modifying their function selector mapping after construction must implement the `IDiamondMutable` interface.

**A `DiamondCut` event must be emitted any time external functions are added, replaced, or removed.**. This applies to all changes, whether through `diamondCut` or not.

```solidity
interface IDiamondMutable {
...
The `diamondCut` function updates any number of functions from any number of facets in a single transaction, and is specified for the purpose of interoperability. Diamond tools, software and user-interfaces should expect and use the standard `diamondCut` function.

```solidity
interface IDiamondCut is IDiamondMutable {
 ...
 * If the `action` is `Replace`, update the function selector mapping for each `functionSelectors` item to the `facetAddress`. If any of the `functionSelectors` had a value equal to `facetAddress`, revert instead.
...
After adding/replacing/removing functions the `_calldata` argument must be executed with `delegatecall` on `_init`. This execution is done to initialize data or setup or remove anything needed or no longer needed after adding, replacing and/or removing functions.
...
See [`Diamond.sol`](../assets/eip-2535/reference/Diamond.sol).
